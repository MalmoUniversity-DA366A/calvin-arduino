...
rStatus CalvinMini::createActor(JsonObject &msg){
  ...
  newActor.count = (int)count.get("data");
...
rStatus CalvinMini::process(int token)
{
  rStatus allOk;
  int8_t pos;
  allOk = FAIL;
...
void CalvinMini::handleToken(JsonObject &msg, JsonObject &reply)
{
  ...
  fifoStatus = process((int)token.get("data"));
...
void CalvinMini::loop()
{
  ...
  while(1)
  {
    ...
    if(socketHandler.anyoneConnected() == 1)                  
    {
      if(socketHandler.recvAllMsg() == 1)                   
      {
        for(int i = 0; i < MAX_NBR_OF_SOCKETS; i++)             
        {
          ...
          {
        	Serial.println("Declaring Buffer");
